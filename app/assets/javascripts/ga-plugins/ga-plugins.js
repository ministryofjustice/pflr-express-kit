...
(function () {
  ...
  ga('require', 'outboundLinkTracker', {
    ...
    hitFilter: function hitFilter(model) {
      ...
      var eventType = model.get('eventAction');
      var eventAction = model.get('eventLabel');
      if (eventType) {
  ...
  ga('require', 'mediaQueryTracker', {
    definitions: [{
      ...
      items: [{ name: 's', media: 'all' }, { name: 'm', media: '(min-width: 740px)' }, { name: 'l', media: '(min-width: 1080px)' }]
    }, {
      name: 'Pixel Density',
      dimensionIndex: dimensions.PIXEL_DENSITY,
      items: [{ name: '1x', media: 'all' }, { name: '1.5x', media: '(min-resolution: 144dpi)' }, { name: '2x', media: '(min-resolution: 192dpi)' }]
    }, {
