...
let { getBlock, getBlockProp, getBlockRaw, setBlock } = blockMethods
...
function recurseBlocks (node) {
  ...
  node.forEach(el => {
    ...
    let checkbox = getBlockProp(el, 'type') === 'checkboxGroup'
    if (checkbox) {
...
if (development) {
  ...
  router.get('/admin/routes', (req, res) => {
    let routes = getBlockProp('routes', 'routes')
  ...
  router.get('/admin/block/:block', (req, res) => {
    ...
    let allProps = Object.keys(block).filter(key => {
      if (key === '_blockType') {
        return false
      }
      if (key.indexOf('x') === 0) {
    ...
    if (allProps.length === 1 && typeof block[allProps[0]] === 'string') {
  ...
  router.get('/admin/block/:block/:prop', (req, res) => {
    ...
    let doesNotExist = getBlock(req.params.block) === undefined
  ...
  router.get('/api/block/:block', (req, res) => {
    res.json(getBlock(req.params.block) || {})
  })
...
function registerRoutes () {
  ...
  let routes = getBlockProp('routes', 'routes') || [] // routesConfig.routes
    ...
    routes.forEach((routeName, index) => {
      ...
      if (route.steps) {
        ...
        route.steps.forEach((step, i) => {
          routesFlattened[step] = Object.assign({}, getBlock(step))
          if (route.steps[i + 1]) {
  ...
  Object.keys(routesFlattened).sort((a, b) => {
    ...
    let routeHandler = (req, res, next) => {
      ...
      let checkNoDependency = (name, skip) => {
        ...
        let depends = getBlockProp(name, 'depends')
        if (depends) {
        ...
        .then(routeOutcome => {
          ...
          if (!errors && req.method === 'POST' && routeInstanceFinal.redirect && req.originalUrl !== routeInstanceFinal.redirect && req.body.updateForm !== 'yes') {
            ...
            nunjucksEnv.addGlobal('getDisplayValue', (name, separator, vals) => {
              ...
              if (options) {
                let type = FormattedMethods.getBlockProp(name, 'type')
                if (type === 'radioGroup') {
                  options.forEach(opt => {
            ...
            res.render(`templates/${template}/${template}`, {
              ...
              if (err) {
                ...
                res.send(rendered)
                if (routeInstanceFinal.static && ENV && ENV !== 'testing') {
                  ...
                  mkdirp(path.resolve('public', 'html', pageDir), mkdirErr => {
                    ...
                    fs.writeFile(path.resolve('public', 'html', pagePath), rendered, writeFileErr => {
                      if (writeFileErr) {
                        ...
                        logger(`Wrote ${pagePath} to /public/html`)
                      }
                    })
                  })
                }
