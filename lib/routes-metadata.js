...
if (development) {
  ...
  router.get('/admin/autofields/reset', (req, res) => {
    ...
    const redirectUrl = req.headers.referer || '/admin/autofields'
  ...
  router.get('/admin/block/:block', (req, res) => {
    ...
    if (block._blockType === 'String' && stringProps.length === 1 && typeof block[stringProps[0]] === 'string') {
    ...
    res.render('admin/block/block', {
      ...
      blockJSONExpanded: JSON.stringify(getBlock(req.params.block) || {}, null, 2),
      stringProps
    })
  })
    ...
    let routeHandler = (req, res, next) => {
      ...
      if (req.method === 'POST') {
        ...
        blocksFound.forEach(el => {
          ...
          if (schema.required && schema._blockType === 'CheckboxGroup') {
            let optionChecked
            if (schema.options) {
          ...
          let validationError
          if (components[schema._blockType] && components[schema._blockType].validate) {
            ...
            validationError = components[schema._blockType].validate(componentValues, schema, prefixedEl)
          } else {
      ...
      routeHandler(routeInstance, Object.assign(
        ...
        .then(routeOutcome => {
          ...
          if (!isFlowchart && !noRedirect && !errors && req.method === 'POST' && routeInstanceFinal.redirect && req.originalUrl !== routeInstanceFinal.redirect && req.body.updateForm !== 'yes') {
            ...
            if (isChange) {
              ...
              if (wizardlastRoute) {
                ...
                if (getBlockProp(wizardlastRoute, 'template') !== 'summary') {
                  let newRedirectRoute = getBlockProp(wizardlastRoute, 'redirect') || routeInstanceFinal.nextStep
                  if (newRedirectRoute) {
            ...
            if (routeInstance.skipRoute || (route.blocks && (!routeInstanceFinal.blocks || !routeInstanceFinal.blocks.length))) {
            ...
            let nunjucksEnv = res.app.locals.settings.nunjucksEnv
            nunjucksEnv.addGlobal('getValue', (name, defaultValue) => {
