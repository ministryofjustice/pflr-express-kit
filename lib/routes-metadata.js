...
if (development) {
  ...
  router.get('/admin/blocks/:blockType/new', (req, res) => {
    ...
    let blockTypeTemplate = `${_blockType}Template`
    if (_blockType === 'Route') {
      blockTypeTemplate = 'View'
    }
    let isaOptions = getKeys(blockTypeTemplate).map(isa => {
      const label = getBlockProp(isa, 'description', isa)
      return {
        value: isa,
        label
      }
    })
    if (!isaOptions.length) {
      isaOptions = undefined
    }
    const blockJSON = {
      _blockType
    }
    if (schemas[_blockType]) {
      const schemaProps = schemas[_blockType].properties
      Object.keys(schemaProps).forEach(key => {
        const propDefault = schemaProps[key].default
        if (propDefault !== undefined) {
          blockJSON[key] = propDefault
        }
      })
    }
  ...
  router.get('/admin/select/block/:block', (req, res) => {
    let searchQuery = req._parsedUrl.search
    let blockTypes
    const blockRef = req.params.block
    const propertyRef = req.query.propertyRef
    const blockType = getBlockProp(blockRef, '_blockType')
    const schema = schemas[blockType]
    let schemaRejects
    if (schema) {
      const schemaProp = schema.properties[propertyRef]
      if (schemaProp) {
        blockTypes = schemaProp.accepts
        schemaRejects = schemaProp.rejects
        if (schemaProp.type === 'array' && schemaProp.items) {
          if (!blockTypes) {
            blockTypes = schemaProp.items.accepts
          }
          if (!schemaRejects) {
            schemaRejects = schemaProp.items.rejects
          }
        }
      }
    }
    if (!blockTypes) {
      blockTypes = blocksDal.getBlockTypes()
                              .filter(type => type)
                              .filter(type => !type.endsWith('Template'))
                              .sort()
    }
    schemaRejects = schemaRejects || []
    blockTypes = blockTypes.filter(key => !schemaRejects.includes(key))
    if (blockTypes.length === 1) {
      res.redirect(`/admin/select-create/${blockTypes[0]}${searchQuery}`)
      return
    }
    res.render('admin/block-select/block-select', { blockTypes, searchQuery })
  })
  router.get('/admin/select/:blockType', (req, res) => {
    const _blockType = req.params.blockType
    let keys = blocksDal.getKeys(_blockType).sort()
    const { blockRef, propertyRef } = req.query
    res.render('admin/select/select', { keys, _blockType, blockRef, propertyRef })
  })
  router.get('/admin/select-create/:blockType', (req, res) => {
    const _blockType = req.params.blockType
    let searchQuery = req._parsedUrl.search
    res.render('admin/select-create/select-create', { _blockType, searchQuery })
  })
  ...
  router.get('/admin/block/:block', (req, res) => {
    const blockId = req.params.block
    let block = getBlockRaw(blockId) || {}
    let blockJSON = block
    let blockType = getBlockProp(blockId, '_blockType') || block._blockType
    let nextRoute
    let previousRoute
    if (blockType === 'Route') {
      const stepsFlat = wizardHierarchy['route:index'] ? wizardHierarchy['route:index'].stepsFlat : []
      const indexSteps = stepsFlat
                          .map(step => step.replace(/(_\d+)+$/, ''))
                          .filter((step, index, array) => {
                            return array.indexOf(step) === index
                          })
      const currentIndex = indexSteps.indexOf(blockId)
      nextRoute = indexSteps[currentIndex + 1]
      if (currentIndex > 0) {
        previousRoute = indexSteps[currentIndex - 1]
      } else if (currentIndex === 0) {
        previousRoute = 'route:index'
      } else if (currentIndex === -1) {
        nextRoute = indexSteps[0]
      }
    }
    const expandedBlock = getBlock(blockId)
    const schema = schemas[blockType]
    const schemaProperties = schema ? schema.properties : {}
    let schemaProps = Object.keys(schemaProperties).map(prop => prop).filter(key => {
      if (key === '_id') {
        return false
      }
      if (key === '_blockType') {
      ...
      if (expandedBlock.excludedProperties && expandedBlock.excludedProperties.includes(key)) {
        return false
      }
      return true
    })
    const initProps = (schema && schema.init ? schema.init : []).filter(key => schemaProps.includes(key))
    const validationProps = (schema && schema.validationProperties ? schema.validationProperties : []).filter(key => schemaProps.includes(key))
    const errorProps = (schema && schema.errorProperties ? schema.errorProperties : []).filter(key => schemaProps.includes(key))
    schemaProps = schemaProps.filter(key => !validationProps.includes(key))
                             .filter(key => !errorProps.includes(key))
    let otherProps = schemaProps.filter(key => !initProps.includes(key))
    ...
    let additionalProps = Object.keys(block).filter(key => {
    ...
    if (!schemaProps.length && additionalProps.length) {
      schemaProps = additionalProps
      additionalProps = []
    }

    const advancedKeys = ['depends', '_isa', 'subblock', 'requiredWhen', 'redirectConditions', 'visuallyhidden', 'template', 'controller', 'extraBlocks']
    let advancedProps = schemaProps.filter(key => advancedKeys.includes(key))
    schemaProps = schemaProps.filter(key => !advancedKeys.includes(key))
    otherProps = otherProps.filter(key => !advancedKeys.includes(key))
    let metaProps = schemaProps.filter(key => key.startsWith('*'))
    schemaProps = schemaProps.filter(key => !key.startsWith('*'))
    otherProps = otherProps.filter(key => !key.startsWith('*'))

    let allProps = [].concat(schemaProps, additionalProps)
    // let type = block.type
    // if (type === 'string') {
    ...
    allProps = allProps.filter((item, index, array) => {
      return array.indexOf(item) === index
    })
    // redirect single value props
    if (!init && block._blockType === 'String' && allProps.length === 1 && typeof block[allProps[0]] !== 'object') {
    ...
    res.render('admin/block/block', {
      ...
      parentBlock: routesFlattened.hasOwnProperty(req.params.block) ? routesFlattened[req.params.block].parent : null,
      init,
      schemaProps,
      allProps,
      initProps,
  ...
  router.post('/admin/block/:block', (req, res) => {
    ...
    let { block, action, cloneId, propName, _isa, init } = req.body
    ...
    if (req.body) {
      const { blockRef, propertyRef } = req.body
      if (blockRef && propertyRef) {
        const blockToAppend = blockRef
        let appendBlock = getBlockRaw(blockToAppend)

        let propertyRefIsArray
        let blockRefSchema = schemas[getBlockProp(blockRef, '_blockType')]
        if (blockRefSchema && blockRefSchema.properties[propertyRef]) {
          propertyRefIsArray = blockRefSchema.properties[propertyRef].type === 'array'
        } else {
          propertyRefIsArray = Array.isArray(appendBlock[propertyRef])
        }
        if (propertyRefIsArray) {
          appendBlock[propertyRef] = appendBlock[propertyRef] || []
          appendBlock[propertyRef].push(blockId)
        } else {
          appendBlock[propertyRef] = blockId
        }
        setBlock(appendBlock._id, appendBlock)
        registerAgain = true
      }
    }
    if (registerAgain) {
    ...
    if (init) {
      redirectUrl += '?init=true'
    }
    res.redirect(redirectUrl)
  })
  const arrayProps = ['blocks', 'steps', 'options', 'items']
  const getInheritedPropRef = (block, prop) => {
    const _isa = getBlockProp(block, '_isa')
    if (!_isa) {
      return
    }
    let blockRef
    let blockProp = getBlockRaw(_isa)[prop]
    if (blockProp !== undefined) {
      blockRef = _isa
    } else {
      blockRef = getInheritedPropRef(_isa, prop)
    }
    return blockRef
  }
  router.get('/admin/block/:block/:prop', (req, res) => {
    ...
    let inheritedValue
    let inheritedBlockReference
    if (propValue === undefined) {
      inheritedValue = getBlockProp(block, prop)
      if (inheritedValue !== undefined) {
        inheritedBlockReference = getInheritedPropRef(block, prop)
      }
    }
    let propType = 'string'
  ...
  router.post('/admin/block/:block/:prop', (req, res) => {
    ...
    if (value !== undefined && rawBlock) {
      ...
      if (propType === 'array') {
        ...
        value = value.filter(item => item !== '').filter(item => JSON.stringify(item) !== '{}')
        if (value.length === 0) {
          value = undefined
        }
...
function registerRoutes () {
  ...
  wizardHierarchy = getRouteHierarchy(routes, routesFlattened)
  ...
  Object.keys(routesFlattened).sort((a, b) => {
    ...
    let routeHandler = (req, res, next) => {
      ...
      if (url !== originalUrl) {
        const testUrl = originalUrl.replace(/\/(edit|flowchart|preview|change)$/, '') || '/'
        if (url !== testUrl) {
          next()
          return
        }
      ...
      let isPreview = !!req.path.match(/\/preview$/)
      ...
      if (!isPreview) {
      // TODO: FIX THIS HORROR WUH?
        if (businessBlocks || routeInstance._isa === 'view:form' || routeInstance._form) {
      ...
      routeHandler(routeInstance, Object.assign(
        ...
        .then(routeOutcome => {
          ...
          let previewData
          if (isPreview) {
            const stepsFlat = wizardHierarchy['route:index'].stepsFlat.map(step => {
              return step.replace(/_\d{1,2}$/, '_1').replace(/_\d{1,2}_1$/, '_1_1')
            }).filter((step, index, array) => {
              return array.indexOf(step) === index
            })
            let previousUrl
            ...
            let nextUrl
            let nextRoute
            console.log(routeInstanceFinal._id, stepsFlat.indexOf(routeInstanceFinal._id))
            const routePosition = stepsFlat.indexOf(routeInstanceFinal._id)
            if (routePosition > 0) {
              previousRoute = stepsFlat[routePosition - 1]
              previousUrl = getRouteUrl(previousRoute)
            } else if (routePosition === 0) {
              previousRoute = 'route:index'
              previousUrl = getRouteUrl(previousRoute)
            }
            nextRoute = stepsFlat[routePosition + 1]
            if (nextRoute) {
              nextUrl = getRouteUrl(nextRoute)
            }
            previewData = {
              nextUrl,
              nextRoute,
          ...
          if (!isPreview && !isFlowchart && !noRedirect && !errors && req.method === 'POST' && routeInstanceFinal.redirect && req.originalUrl !== routeInstanceFinal.redirect && req.body.updateForm !== 'yes') {
            if (saveReturn) {
            ...
            if (routeInstance.skipRoute || (req.method === 'POST' && routeInstance.noblocks) || (!isEdit && !isPreview && route.blocks && (!routeInstanceFinal.blocks || !routeInstanceFinal.blocks.length))) {
            ...
            nunjucksEnv.addGlobal('previewData', previewData)
