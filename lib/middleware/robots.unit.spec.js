...
test('When using default robots configuration and a user agent requests robots.txt', t => {
  ...
  const { assertNextCalled, assertStatusCode, assertBody, assertHeaders } = invokeMiddleware(defaultRobots, t, '/robots.txt')
...
test('When using default robots configuration and a user agent requests any other resource', t => {
  ...
  const { assertNextCalled, assertStatusCode, assertHeaders } = invokeMiddleware(defaultRobots, t, '/foo')
...
test('When using custom configuration and a user agent requests robots.txt', t => {
  ...
  const { assertNextCalled, assertBody, assertHeaders } = invokeMiddleware(customRobots, t, '/robots.txt')
...
test('When using custom configuration and a user agent requests any other resource', t => {
  ...
  const { assertNextCalled, assertHeaders } = invokeMiddleware(customRobots, t, '/foo')
...
test('When robots are disallowed and a user agent requests robots.txt', t => {
  ...
  const { assertNextCalled, assertBody, assertHeaders } = invokeMiddleware(disallowRobots, t, '/robots.txt')
...
test('When robots are disallowed and a user agent requests any other resource', t => {
  ...
  const { assertNextCalled, assertHeaders } = invokeMiddleware(disallowRobots, t, '/foo')
