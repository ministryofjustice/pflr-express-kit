...
function setFormat (baseArgs, options) {
  ...
  function reformat (value, args) {
    if (value.match(recurseMatch)) {
      value = value.replace(recurseMatch, (m, m1) => {
        ...
        let keyProp = 'value'
        let keyArgs = Object.assign({}, args)
        if (keyParams.includes(';;;')) {
          let keysParamsChunks = keyParams.split(';;;')
          keyParams = keysParamsChunks[0].trim()
          let newKeyArgs = keysParamsChunks[1].trim().split(';;')
          newKeyArgs.forEach(argSrc => {
            const argChunks = argSrc.trim().split('=')
            keyArgs[argChunks[0]] = argChunks[1]
          })
        }
        if (keyParams.includes('#')) {
          let keysParamsChunks = keyParams.split('#')
        ...
        let nestedValue = getFormattedProp(keyName, keyProp, undefined, {args: keyArgs})
        return nestedValue
      })
  ...
  function format (value, args) {
    ...
    if ((value.indexOf('{') === -1 && value.indexOf('~~') === -1 && value.indexOf('$index') === -1 && value.indexOf('$counter') === -1) && !value.match(recurseMatch)) {
      return value
    }
    args = args || baseArgs
    ...
    if (value.match(/\$\$index/)) {
      value = value.replace(/\$\$index/g, 'app:route:_index')
    }
    if (value.match(/\$index/)) {
      value = value.replace(/\$index/g, args['app:route:_index'])
    }
    if (value.match(/\$\$dynamicIndex/)) {
      value = value.replace(/\$\$dynamicIndex/g, 'app:route:dynamicIndex')
    }
    if (value.match(/\$dynamicIndex/)) {
      value = value.replace(/\$dynamicIndex/g, args['app:route:dynamicIndex'])
    }
    if (value.match(/\$\$counter/)) {
      value = value.replace(/\$\$counter/g, 'app:route:multiple_counter')
    }
    if (value.match(/\$counter/)) {
      value = value.replace(/\$counter/g, args['app:route:multiple_counter'])
    }
    // replace straight dynamic substitutions
    try {
      if (value.match(/~~.+~~/)) {
  ...
  function getFormattedError (block, options) {
    ...
    formattedError = format(formattedError, {
      ...
      minLength: getBlockProp(block, 'minLength'),
      maxLength: getBlockProp(block, 'maxLength'),
      pattern: getBlockProp(block, 'pattern'),
      name: block,
