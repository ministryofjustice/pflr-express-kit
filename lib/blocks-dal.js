...
function processBlocks () {
  ...
  getAll('multiple', true).forEach(key => {
    ...
    const newBlocks = []
    const start = block.multiple_start !== undefined ? block.multiple_start : 1
    block.multiple_start = start
    const max = block.multiple_max || start || 1
  ...
  routes.forEach(key => {
    const block = blocks[key]
    if (block._blockType === 'Route' && block._hasMany) {
      const generatedBlocks = []
      const start = block._start || 1
      const max = block._max || start
      for (let counter = 1; counter <= max; counter++) {
        const countedRoute = Object.assign({}, block)
        countedRoute._counter = countedRoute._counter || countedRoute._id.replace(/route:/, '')
        countedRoute._index = counter
        countedRoute._id += '_' + counter
        countedRoute.url += '/' + counter
        countedRoute._suffix = '_' + counter
        countedRoute._prefix = countedRoute._counter + '_' + counter + '_'
        if (counter > start) {
          countedRoute.depends = [
            {
              [countedRoute._counter]: `>=${counter}`
            }
          ]
        }
