  ...
  routes.forEach(key => {
    ...
    if (block._blockType === 'Route' && block.multiple) {
      ...
      for (let counter = 1; counter <= max; counter++) {
        ...
        if (counter > min) {
          countedRoute.depends = countedRoute.depends || []
          countedRoute.depends.push({
        ...
        if (steps) {
          console.log('HAS STEPS', countedRoute._id)
          const multipleSteps = []
          steps.forEach(step => {
            const stepBlock = Object.assign({}, blocks[step])
            ...
            if (counter > min) {
              stepBlock.depends = stepBlock.depends || []
              console.log('steps multipleCounter', stepBlock.multiple_counter, multipleCounter)
              stepBlock.depends.push({
                [stepBlock.multiple_counter]: `>=${counter}`
              })
            }
            blocks[stepBlock._id] = stepBlock
            multipleSteps.push(stepBlock._id)
          })
          countedRoute.steps = multipleSteps
        }
      }
      if (block.multiple_summary) {
        const multipleSummary = block.multiple_summary
        const summaryBlock = Object.assign({}, blocks[multipleSummary])
      ...
      if (block._id === 'route:applicant-details') {
        console.log(blocks[block._id + '_1'])
      }
