...
function processBlocks () {
  ...
  function extendBlock (key) {
    ...
    if (block._isa) {
      ...
      if (!extendedBlock) {
        throw new Error(`No block found for _isa ${extendedKey} in ${key}`)
      }
  ...
  Object.keys(blocks).forEach(key => {
    ...
    if (block._blockType === 'Route' && block.block) {
      ...
      if (!block.heading) {
        block.heading = reffedBlock.label || reffedBlock.heading
        if (reffedBlock.visuallyhidden !== false) {
    ...
    if (keyBlock.steps && keyBlock.steps_heading) {
      const updateSteps = (steps, stepsHeading) => {
        steps.forEach(step => {
          ...
          if (stepBlock && !stepBlock.section_heading) {
      ...
      for (let counter = 1; counter <= max; counter++) {
        ...
        const updateSteps = (routeToUpdate, depth = 0) => {
          ...
          if (steps) {
            ...
            steps.forEach(step => {
              if (!blocks[step]) {
                throw new Error(`No such step ${step}, reffed by ${routeToUpdate._id}`)
              }
